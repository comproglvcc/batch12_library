...
function ShowAdminDelDocWin(docId)
{
	if (_adimDelDocWin == null)
	{
		...
		_adimDelDocWin.onClose = function() {
			if ($j('#flashContainerParent').length>0) {
				$j('#blackbg').hide();
			}
		};
...
function doSearch(type, altCntrl){
	...
	{
	    // var q=escape(txtCntrl.value.replace(/-/g,"--").replace(/ /g,"-").replace("-","~"));
	    var q=escape(txtCntrl.value.replace("-","~").replace(/ /g,"-"));
...
function clearSearch(el)
{
    ...
    if (el.value.indexOf('Search More') > -1) { el.value = ""; }
}		
...
function SetLanguage(ltype,displayLang)
{
	...
	switch (pageName) {
	    case "/search/default.aspx":
	        dsmetrics.track(new DSMActivity("refine-category-language", "", 10, 71, 8, [new DSMOtherInfo("value", ltype)]));
...
function SetDocType(dtype)
{
	...
	switch (pageName) {
	    case "/search/default.aspx":
	        dsmetrics.track(new DSMActivity("refine-category-type", "", 10, 71, 8, [new DSMOtherInfo("value", dtype)]));
	        setTimeout("window.location.reload();", 300);
	        break;
	    default:
	        window.location.reload();
...
function setItemsPerPage(value){
    ...
    switch (pageName) {
        case "/search/default.aspx":
            dsmetrics.track(new DSMActivity("results-per-page", "", 10, 71, 8, [new DSMOtherInfo("value", value)]));
            setTimeout("window.location.reload();", 300);
            break;
        default:
            window.location.reload();
...
function BookmarkDoc(docID) {
    //declare object
    var oAjax = new AjaxObject101();
    //set response type
    oAjax.returnXml = false;
    oAjax.funcDone = BookmarkComplete;
    //set method delegates
    oAjax.sndReq('GET', '/docs/SaveBookmark.ashx', 'doc_id=' + docID);

}
function BookmarkComplete(data) {
    //ShowGenWin('Document has been bookmarked.');

    ShowBookmarked();
}
function ShowBookmarked() {
    //change em text
    $j('#emBkmrk').text('Bookmarked').css('margin-left', '-10px');

    //show little green checkbox on success
    $j('.fdss-bookmark').css('background', 'url(/i/doc/greencheck.png) no-repeat');
    $j('.fdss-bookmark').css('float', 'left');
    $j('.fdss-bookmark').css('height', '21px');
    $j('.fdss-bookmark').css('width', '21px');
    $j('.fdss-bookmark').css('top', '10px');
    $j('.fdss-bookmark').css('left', '16px');
}

function LikeDoc(docID) {
    //declare object
    var oAjax = new AjaxObject101();
    //set response type
    oAjax.returnXml = false;
    //oAjax.funcDone = LikeComplete;
    //set method delegates
    oAjax.sndReq('GET', '/docs/SaveLike.ashx', 'doc_id=' + docID);

}
function LikeComplete(data) {
    ShowGenWin('You liked this document.');
}
