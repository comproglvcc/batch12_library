...
function ShowLogin(){
	...
	el.innerHTML = c;
	var header = $j("#header");
	$j(header).find("#login-info").remove();
	$j(header).append(el);
	$j("#banner").hide();
}
...
function TopLogin(){
	...
	var keep = false;
	if ( $j('#chkRemember').is(":checked")) {keep=true; }
	//declare object
...
function showTipCVV(sender){showImageToolTip(sender,'/i/upload/cvv-help.png','cvv-tip1',200, -30);}
...
function showImageToolTip(sender, imgSrc, id, top, left) {
	...
	$j(tip).css({
		'display' : 'block',
		'top' : (sPos[1]-defaultTop) + 'px',
		'left' : (sPos[0]+defaultLeft) + 'px'
	});
...
function setupIframe(tip,divId){
	...
	visibility : 'visible',
	  position : 'absolute',
	    ...
	    zIndex : tip.css('zindex') - 1,
	     width : tip.width(),
	    height : tip.height(),
	       top : tip.css('top'),
	      left : Math.round( tip.css('left').replace('px','') ) + 10
		});
}

function hideImageToolTip(sender, id) {
	if($j.browser.msie && $j.browser.version==6.0 && myIFrame!=null) {
...
function DisableScreen2(id,color,opacity){

	var id      = id      || 'blackbg';
	var color   = color   || '000000';
	var opacity = (opacity && typeof(opacity) === "number" && opacity <= 1) ? opacity : .7 || .7;
	var ieOpacity = 100 * opacity;

	var dom = (id.substring(0) === "#" || id.substring(0) === ".") ? id : "#" + id;
	color = (color.substring(0) === "#") ? color : "#" + color;

	if ($j(dom).length != 0) {
		$j(dom).css({ 'display': 'block' });
		return; 
	}

	$j("body").append("<div id='" + id + "'></div>");
...
function DisableScreen(id,color,opacity){
    ...
    if ($j(cssId).length != 0) {
        $j(cssId).css({ 'display': 'block' });
     return; }

     var ieOpacity =parseFloat(opacity);
     if (ieOpacity == "NaN") {

         ieOpacity = 70;
     }
     else {
         ieOpacity = ieOpacity * 100;
     }
...
function trackAd(pageType, action, activity, destination, adID, docID, term) {
	...
	if (typeof dsmetrics != 'undefined') {
	    dsmetrics.track(new DSMActivity("ad-click", "", dsmetrics.ActivityObjType.InternalAd, pageType, dsmetrics.ActivityActionType.Click, [new DSMOtherInfo("ad_id", adID), new DSMOtherInfo("doc_id", docID)]));
	}
}

function showMoreTxt(linkID,moreTxtID) {
	$j("#" + linkID).hide();
	$j("#" + moreTxtID).css("display","inline");
// document.getElementById(linkID).style.display = 'none'; // document.getElementById(moreTxtID).style.display = 'inline';
}
...
function switchResultsLayout(layout,dontSetCookie) {
	...
	if (getEl('results-layout')) {
		...
		if(getEl('sel-text')) getEl('sel-text').className = '';
		if (getEl('sel-thumb')) getEl('sel-thumb').className = '';
		if (getEl('sel-thumb-text')) getEl('sel-thumb-text').className = '';
		if (getEl('sel-text2')) getEl('sel-text2').className = '';
		if (getEl('sel-thumb2')) getEl('sel-thumb2').className = '';
		if (getEl('sel-thumb-text2')) getEl('sel-thumb-text2').className = '';
		if (getEl('sel-' + layout)) getEl('sel-' + layout).className = 'sel';
		if (getEl('sel-' + layout + '2')) getEl('sel-' + layout + '2').className = 'sel';
    ...
    switch(pageName)
    {
        case "/search/default2.aspx":
            dsmetrics.track(new DSMActivity("results-view-type", "", 10, 116, 8, [new DSMOtherInfo("value", layout)]));
        break;
        case "/search/default.aspx":
            dsmetrics.track(new DSMActivity("results-view-type", "", 10, 71, 8, [new DSMOtherInfo("value", layout)]));
        break;
    }
}
...
function HideSpotlight() {
	...
	$j('#spotlight').css("display", 'none');
	$j('#open-spotlight').css("display", 'block');
}

function OpenSpotlight() {
	setCookie('displaySpotlight',1,3650);
	$j('#spotlight').css("display", 'block');
	$j('#open-spotlight').css("display", 'none');
}
...
function CollAddDocDone(result){
	...
	if(result.status==1){
		...
		var divToDisplay = $j('#coll' + docid);
		if($j(divToDisplay).html()){
			$j(divToDisplay).html('<p>Added!</p>');
		}
	}
	else {
...
function CollRemDocDone(result){
	...
	if(result.status==1){
		var docid = result.message;
		var divToDisplay = $j('#d' + docid);
		if($j(divToDisplay).html()){
			$j(divToDisplay).slideUp();
...
function showLargeImages() {
	var selectThumb = $j('#sel-thumb'),
	showBigImages = ($j(selectThumb).hasClass("sel") || $j("a.selected>#sel-thumb").length > 0);
	if(showBigImages) {
		$j('#results-layout .img_206').each(function(){
			...
			if(currentSrc == '' || currentSrc == null) {
				...
				if(idArray.length === 3) {
					$j(this).attr('src', (imgHost+"/thumb/"+idArray[1]+"/"+idArray[2]+".png") );
				}
			}
		});
	}
...
$j(document).ready(function() {
	$j("#txtSearch").autocomplete("/search/AutoComplete.ashx",
	{ minChars: 4, max: 8, delay: 50, width: '325px', selectFirst: false, cacheLength: 0 });
	if ((getEl('results-layout') && getEl('sel-thumb') && getEl('sel-thumb').className == 'sel') || $j("a.selected>#sel-thumb").length > 0){ showLargeImages(); }
});

function PingServer(url) { $j.getJSON(url + "&jsoncallback=?", function(data){return true;}); }
function OpenCreditOverlay() { $j('.creditOverlay').show(500); }
function OpenCreditOverlayMulti(itemId) { $j('#creditOverlay_'+itemId).show(500); }
function CloseCreditOverlay() { $j('.creditOverlay').hide(500); }
function BookmarkDocInList(el, docID) { $j.getJSON('/docs/SaveBookmark.ashx?doc_id=' + docID, function(data) { $j(el).html('Bookmarked'); }); }
function BookmarkDocInList2(docID) {$j.getJSON('/docs/SaveBookmark.ashx?doc_id=' + docID, function(data) { $j("#b" + docID).html('Bookmarked'); }); }

function loadViewer(elementId,docID, memID, width, height, printOnLoad, viewerLocation, key, pass,showBuyButton) {
		$j("#"+elementId).empty(); 
		...
		var nobuybutton = 1-showBuyButton; //false meaning
		
		var embed = '<object id="_ds_' + docID + '" name="_ds_' + docID + '" width="' + width + '" height="' + height + 
		'" type="application/x-shockwave-flash" data="' + url + '"><param name="FlashVars" value="doc_id=' + docID + 
		'&mem_id=' + memID + '&showrelated=0&showotherdocs=0&doc_type=pdf&allowdownload=0&nobuybutton='+nobuybutton+'&printdoc=' + printOnLoad +
		'" /><param name="movie" value="' + url + '"/><param name="allowScriptAccess" value="always" /><param name="allowFullScreen" value="true" /></object>';
			$j("#"+elementId).append(embed);
	};

	function addGhostTextToInputMeta($obj, color1, color2) {
		$obj.focus(function() {
			if ($j(this).val() == $j(this).attr('title')) {
				$j(this).val('').css("color",color1);
			}
		});
		$obj.blur(function() {
			if ($j(this).val() == '') {
				$j(this).val($j(this).attr('title')).css("color",color2);
			}
		});
		$obj.blur().css("color",color2);		
	}

	function addGhostTextToInput($obj) {
		addGhostTextToInputMeta($obj, "black", "#BBB");
	}


function SubmitFeedBack(docID, vote) {
	$j.get("/handlers/UserFeedback.ashx", { doc_id: docID, helpfull: vote });
}
function SubmitPageFeedBack(vote) {
	$j.get("/handlers/UserFeedback.ashx", { doc_id: docID, helpfull: vote, url: escape(window.location.href) });
}

$j.fn.outerHTML = function() {
	return $j(this).clone().wrap('<div></div>').parent().html();
}
