...
function setPadding() {
	...
	if (!fixedWidth) {
		...
		document.getElementById('logo').style.paddingLeft = padValue;
		document.getElementById('login').style.paddingRight = padValue;
...
function SetRelatedDocs() { console.error("SetRelatedDocs was removed, please remove all calls to it"); } 
...
function resizeViewer() {
	if (!quickview) {
		...
		if (document.getElementById('flashContainer')) {
		    ...
		    textHeight = document.getElementById('viewer-area').clientHeight;
		    docInfo.style.paddingTop = textHeight + 'px'
		}

		/* removed, js set width 
		if ($('related-doc')) {
			SetRelatedDocs();
		}
		*/

		var nWidth = jQuery('.nav-inner').width();
		nWidth = nWidth * 1;
		jQuery('.nnav_repeating').width(nWidth - 11);
	}
}

function resizeTextViewerArea() {
    var docInfo = document.getElementById('doc-info');
    textHeight = document.getElementById('viewer-area').clientHeight;
    docInfo.style.paddingTop = textHeight + 'px';
}
...
function showDocumentDID(docID, memID, documentType, rev, docError, width, height, isStg, isPaid, showrelated, showotherdocs, searchTerm, key, pass, templateID, adType) {
	showDocumentDID(docID, memID, documentType, rev, docError, width, height, isStg, isPaid, showrelated, showotherdocs, searchTerm, key, pass, templateID, adType, "DSViewer.2.3.65.swf", 0,1);
}
function showDocumentDID(docID, memID, documentType, rev, docError, width, height, isStg, isPaid, showrelated, showotherdocs, searchTerm, key, pass, templateID, adType, viewerLocation, noBuyButton, overideURL, iframesrc, buttonTypes, buttonObject) {
	...
	if (buttonTypes == undefined) { buttonTypes = "1"; }
	...
	//'flashvars', 'iframeurl=' + escape('/docs/viewer-ads.htm') + '&ad_type=' + _adType + '&doc_id=' + docID + '&mem_id=' + memID + '&doc_type=' + documentType + '&embed=0&nobuybutton=' + noBuyButton + '&revision=' + rev + '&showrelated=' + showrelated + '&showotherdocs=' + showotherdocs + '&fullScreenBrowser=1&buttontype=2&buttonlink=' + escape('https://' + window.location.host + '/cart/AddDocument.ashx?docID=' + docID) + suffix
		'flashvars', 'iframeurl=' + escape(iframesrc) + '&ad_type=' + _adType + '&doc_id=' + docID + '&mem_id=' + memID + '&doc_type=' + documentType + '&embed=0&nobuybutton=' + noBuyButton + '&revision=' + rev + '&showrelated=' + showrelated + '&showotherdocs=' + showotherdocs + '&fullScreenBrowser=1&isMaster=1&buttontype=2&buttonviewtype=' + buttonTypes + '&buttonlink=' + escape(buttonURL) + suffix + '&buttonObject='+ buttonObject 
		); //end AC code
...
function parseResults(ga, fUrl) {
    ...
    //_ds_ts_ads = { term: adsSearchTerm, sponsored_text: 'Ads by Google', feedback_url: fUrl, items: list, related: related };
	//_ds_fa_ads = CreateHpAd(adsSearchTerm, "http://www.hpdirect.com", related);
...
function CreateHpAd(adsSearchTerm, fUrl, related) {
    var list = [];
    var reuturnVal = null;

    list[0] = { title: 'Print this Document for Less', line1: 'Save $15 on HP\'s full line of ink and', line2: 'toner cartridges. FREE next day shipping.', url: 'http://bit.ly/go3sbl', displayurl: 'www.hpdirect.com' };
    list[1] = { title: 'Affordable way to print this document', line1: 'Save 10% on ink and toner everyday.', line2: 'Sign up for HP\'s My Print Rewards.', url: 'http://bit.ly/go3sbl', displayurl: 'www.hpdirect.com' };
  

    reuturnVal = { term: adsSearchTerm, sponsored_text: 'Sponsored by HP', feedback_url: fUrl, items: list, related: related };
...
function SetH1Clickable(DocumentName, url) {
	$j("#PageDocTitle").each( function() {
		if($j(this).hasClass("h1-paid")) {
			$j(this).addClass("h1-clickable").click( function() { 
				
				if (parseInt(CurrentUserID) <= 0){
					PingServer('/cart/AddDocument.ashx?docID=' + DocumentID + '&redirect=0');
					...
					if (url != ""){
						...
						location.href = '/cart/AddDocument.ashx?docID=' + DocumentID;
					}
				}
			});
		}
		else if($j(this).hasClass("h1-free")) {
			$j(this).addClass("h1-clickable").click( function() { 
				location.href = '/search/' + escape(DocumentName); 
			});
		}
	});
}
...
$j(function() {
try {
	$j(".credicon").explainTooltip(DefaultExplainTooltipTexts.Credits);
	$j(".dolicon").explainTooltip(DefaultExplainTooltipTexts.Credits);
	$j(".dpremicon").explainTooltip(DefaultExplainTooltipTexts.DocstocPremium);
