...
class TransactionUtils{
      ...
      public static function addTransaction($book_access, $borrower, $issue, $due){
	   ...
	   $sql = "insert into transaction(book_access_num, borrowers_id, date_issue, date_due) values('{$book_access}','{$borrower}','{$issue}','{$due}')";
       echo $sql;
	   $result = mysql_query($sql) or die("Error in your Query");
       $id = mysql_insert_id();	   
		$transaction = "select * from transaction where trans_id ={$id}";		
	  ...
	  public static function returnBook($id, $return){
	   ...
	   $update_remaining_stock = mysql_query("update books set remaining_stock='{$remaining_stock}' where book_id={$book_id}");
		if(mysql_affected_rows()>0){
             return header("location:readTransaction.php?id={$id}");
      }else{
             return false;
      }
	...
	public static function readTransaction(){
       mysql_connect('localhost', 'root');
       mysql_select_db('library_2012');
       $result = mysql_query("select * from transaction t join users u on(u.student_id=t.borrowers_id)
               join book_status bs on(bs.accession_num=t.book_access_num) join books bk on(bk.book_id=bs.book_id) where bs.status = 'out' order by t.trans_id DESC ");
	  ...
	  public static function borrower_unfinish($id){
		   mysql_connect('localhost', 'root');
		   mysql_select_db('library_2012');
		   $result = mysql_query("select * from transaction t join borrowers b on(b.student_id=t.borrowers_id)
				   join book_status bs on(bs.accession_num=t.book_access_num)
				   join books bk on(bk.book_id=bs.book_id) where b.student_id='{$id}' AND t.date_returned=' '");
		   $res = array();
	   ...
	   public static function borrower_finish($id){
       mysql_connect('localhost', 'root');
       mysql_select_db('library_2012');
       $result = mysql_query("select * from transaction t join borrowers b on(b.student_id=t.borrowers_id)
               join book_status bs on(bs.accession_num=t.book_access_num)
