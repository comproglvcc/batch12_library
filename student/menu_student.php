...
if (!empty($_POST['submit'])){
	...
	if ($submit){
		$check=BorrowerUtils::check_exist($snumber);
			if ($check === True){
				$_SESSION['number']=$snumber;
				$ses_num = $_SESSION['number'];
				$result = " You are registered! "."<a href=student_details.php?stud_id=".$ses_num.">View Details</a>";
			}else{
				$result = " You are not yet registered!";
					}
